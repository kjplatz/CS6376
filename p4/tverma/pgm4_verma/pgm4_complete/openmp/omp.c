...
#define THRESHOLD 256               /* Max block size allowable in a process (node). */
#define PROC_SIZE 2048              /* Block size in a process. This value equals N/sqrt(P), where N is the dimension of the three matrices. */
#define ROOT_PROC 0                 /* Variable to denote Process 0. */

/* Definitions for printing the matrix. */
...
int main(int argc, char** argv)
{
	...
	int i = 0, j = 0, m = 0;                         /* Indices for traversing in loops. */
	...
	int a_0 = mesh_coords[0];
	int a_1 = mesh_coords[1];


	/* There are separate loops to avoid conflicts between matrices while parallelizing using OpenMP and OpenACC. */
	...
	for (i = 0; i < PROC_SIZE; i++)
		for (j = 0; j < PROC_SIZE; j++)
			A[i][j] = (a_1*PROC_SIZE + j) - (a_0*PROC_SIZE + i);

	#pragma omp parallel for
	for (i = 0; i < PROC_SIZE; i++)
		for (j = 0; j < PROC_SIZE; j++)
			B[i][j] = N - (a_1*PROC_SIZE + j) + (a_0*PROC_SIZE + i);
	...
	if (rank == ROOT_PROC)                                                                           /* Print results. */
	{
		...
		printf("Performance = %2.4lf GFLOPS. \n", (2*N*N*N)/(1000000000.0*total_time));
		printf("-------------------------------------------------------------------------------------------------------------\n");
	}
